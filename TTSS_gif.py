...
u_original = np.load(".\\data_ttss\\u.npy")
y_original = np.load(".\\data_ttss\\y.npy")
s = np.load(".\\data_ttss\\s.npy")
...
def get_DeepONet_data(u_original, y_original):
    Power, HTC, Die_xy, Die_z, t_Diel, t_Insulator, K_Diel, TTSS_value, y_train = \
        u_original[:, 0:1], u_original[:, 1:2], u_original[:, 2:3], \
        u_original[:, 3:4], u_original[:, 4:5], u_original[:, 5:6], \
        u_original[:, 6:7], u_original[:, 7:8], y_original[:, 0:1]
    ...
    print('TTSS_value_n: ', TTSS_value_n)

    u_train = np.hstack((power_mag_n, HTC_n, Die_xy_n, \
                         Die_z_n, t_Diel_n, t_Insulator_n, K_Diel_n, TTSS_value_n))

    print('y_train.shape[0]//unit_size is: ', y_train.shape[0] // unit_size)
...
filename_max = '.\\model_ttss\\model_max.sav'
interpreter_maxT = pickle.load(open(filename_max, 'rb'))

file_name_DeepONet = '.\\model_ttss\\model.sav'
...
left_space = 50
top_space = 150
org_Power = (left_space, top_space)
org_HTC = (left_space, top_space + legend_space)
org_Die_xy = (left_space, top_space + legend_space*2)
org_Die_z = (left_space, top_space + legend_space*3)
org_t_Diel = (left_space, top_space + legend_space*4)
org_t_Insulator = (left_space, top_space + legend_space*5)
org_K_Diel = (left_space, top_space + legend_space*6)
...
with imageio.get_writer('./movie.gif', mode='I') as writer:
    for k in range(test_num):
        ...
        Final_temp = Final_temp * 25
        y_DeepONet = (y_DeepONet[:400, :] ) / y_DeepONet[:400, :].max() * num_pixel + 10
        Final_temp = num_pixel - Final_temp[:400, :]*1
