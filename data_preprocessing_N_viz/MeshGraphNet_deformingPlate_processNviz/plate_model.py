...
class Model(snt.AbstractModule):
  ...
  def _build_graph(self, inputs, is_training):
    ...
    # print('velocity is: ', velocity.eval(session=tf.Session())[:10])
    # print('node_type is: ', node_type.eval(session=tf.Session())[:10])
    # raise
    ...
    print('relative_world_pos is: ', relative_world_pos.eval(session=tf.Session())[:10])
    print('relative_mesh_pos is: ', relative_mesh_pos.eval(session=tf.Session())[:10])
    print('edge_features is: ', edge_features.eval(session=tf.Session())[:10])
    # print('tf.norm(relative_world_pos, axis=-1, keepdims=True) is: ', tf.norm(relative_world_pos, axis=-1, keepdims=True))
    raise
