...
# mlsolver_output_x = np.load('./meshgraphnets/data/deforming_plate/mlsolver_x.npy')
...
case_id = 6 # 3 is full plate
case_id_e = case_id + 1
for i, (c, m, n, w, p, t, s) in enumerate(zip(cells, mesh_pos, node_type, world_pos, prev_world_pos, target_world_pos, stress)):
# for i, (c, m, n, w, p, t, s) in enumerate(zip(cells[case_id:case_id_e], mesh_pos[case_id:case_id_e],\
# 								node_type[case_id:case_id_e],\
# 								world_pos[case_id:case_id_e], prev_world_pos[case_id:case_id_e],\
# 								 target_world_pos[case_id:case_id_e], stress[case_id:case_id_e])):
	if i not in full_plate_idx:
		continue
	else:
		print('i: ', i)
		count+=1
	print('length of m,n,w,p,t,s: ', len(m),len(n),len(w),len(p),len(t),len(s))
	m_pos_np = np.asarray(m)
	n_type_np = np.asarray(n)
	w_pos_np = np.asarray(w)
	t_pos_np = np.asarray(t)
	stress_np = np.asarray(s)
	...
	with imageio.get_writer('./meshgraphnets/figs/' + f'movie_output_{i}.gif', mode='I') as writer:
############################  Find the max transition ###########################################
############################  Find the max transition ###########################################
############################  Find the max transition ###########################################
		...
		print('m_pos_np shape: ', m_pos_np.shape)
		print('n_type_np shape: ', n_type_np.shape)
		print('w_pos_np shape: ', w_pos_np.shape)
		print('t_pos_np shape: ', t_pos_np.shape)
		# print(np.unique(n_type_np))
		# raise
		bc_idx_process = (n_type_np[0, :, 0] == 3).flatten()
		n_type_np[:, bc_idx_process, 0] = 0 ## Change bc to Plate

		plate_idx_process = n_type_np[0, :, 0] == 0 ## Plate
		print('m_pos_np[0, plate_idx_process, 0]: ', m_pos_np[0, plate_idx_process, 0].shape)

		obs_idx_process = n_type_np[0, :, 0] == 1 ## obstacle


		print('n_type_np equal?: ', np.sum(n_type_np[0,:,0]==n_type_np[200,:,0]))
		print('w_pos_np shape: ', w_pos_np.shape)
		plate_moving_delta_process = t_pos_np - w_pos_np
		delta_transitions = plate_moving_delta_process[1:,:,:] - plate_moving_delta_process[:-1,:,:]
		print('delta_transitions shape: ', delta_transitions.shape)

		# delta_transitions = 
		delta_transitions_abs = np.abs(delta_transitions)
		print('delta_transitions_abs shape: ', delta_transitions_abs.shape)

		delta_transitions_abs_x = delta_transitions_abs[:, :, 0]
		delta_transitions_abs_y = delta_transitions_abs[:, :, 1]
		delta_transitions_abs_z = delta_transitions_abs[:, :, 2]
		delta_transitions_abs_x_avg = np.average(delta_transitions_abs_x, axis=-1)
		delta_transitions_abs_y_avg = np.average(delta_transitions_abs_y, axis=-1)
		delta_transitions_abs_z_avg = np.average(delta_transitions_abs_z, axis=-1)
		delta_transitions_abs_avg = delta_transitions_abs_x_avg + delta_transitions_abs_y_avg +\
									delta_transitions_abs_z_avg
		delta_transitions_abs_max_idx = np.argmax(delta_transitions_abs_avg)
		start_time_step = delta_transitions_abs_max_idx - 8
		end_time_step = delta_transitions_abs_max_idx + 8
		# print('delta_transitions_abs_max_idx: ', delta_transitions_abs_max_idx)
		# raise
############################  Find the max transition ###########################################
############################  Find the max transition ###########################################
############################  Find the max transition ###########################################
		...
		for time_step in range(start_time_step, min(end_time_step, 398), 1):
...
###########################   Scale m_np_step to make z thickness to be the same as width ##########
			...
			obs_m_pos_step = m_np_step[obs_idx_step]
			obs_w_pos_step = w_np_step[obs_idx_step]
			obs_tgt_pos_step = t_np_step[obs_idx_step]
...
############################  Data generation for ML Solver output ##############################
			...
			obs_moving_delta_step = obs_tgt_pos_step - obs_w_pos_step
...
############################  Visualizing the graphs ##########################################
			...
			# im = ax.scatter(plate_w_pos_step[:,0], plate_w_pos_step[:,1], plate_w_pos_step[:,2],\
			#  c=plate_moving_delta_step[:,0].flatten(), cmap='jet')
			# ax.title.set_text(f'plate x translation \n timestep: {time_step}')
			# ax.set_box_aspect([np.ptp(i) for i in (plate_w_pos_step[:,0], plate_w_pos_step[:,1],\
			#  plate_w_pos_step[:,2])])  # equal aspect ratio
			...
			#  c=obs_moving_delta_step[:,0].flatten(), cmap='jet')
			# ax.title.set_text(f'obs x translation \n timestep: {time_step}')
			# ax.set_xlim(0, 0.25)
			# ax.set_ylim(0, 0.5)
			# ax.set_zlim(0, 0.2)
			# ax.set_box_aspect([np.ptp(i) for i in (plate_w_pos_step[:,0], plate_w_pos_step[:,1],\
			...
			# im = ax.scatter(obs_w_pos_step[:,0], obs_w_pos_step[:,1], obs_w_pos_step[:,2],\
			#  c='red')
			# ax.title.set_text(f'plate and obs')
			...
			ax = fig.add_subplot(1, 3, 1, projection='3d')
			im = ax.scatter(plate_w_pos_step[:,0], plate_w_pos_step[:,1], plate_w_pos_step[:,2],\
			 c=plate_moving_delta_step[:,2].flatten(), cmap='jet')
			ax.title.set_text(f'plate z translation \n timestep: {time_step}')
			ax.set_box_aspect([np.ptp(i) for i in (plate_w_pos_step[:,0], plate_w_pos_step[:,1],\
			 plate_w_pos_step[:,2])])  # equal aspect ratio
			cbaxes = fig.add_axes([0.28, 0.25, 0.02, 0.25])
			plt.colorbar(im, cax = cbaxes)
			...
			ax = fig.add_subplot(1, 3, 2, projection='3d')
			im = ax.scatter(obs_w_pos_step[:,0], obs_w_pos_step[:,1], obs_w_pos_step[:,2],\
			 c=obs_moving_delta_step[:,2].flatten(), cmap='jet')
			ax.title.set_text(f'obs z translation \n timestep: {time_step}')
			ax.set_xlim(0, 0.25)
			ax.set_ylim(0, 0.5)
			ax.set_zlim(0, 0.2)
			ax.set_box_aspect([np.ptp(i) for i in (plate_w_pos_step[:,0], plate_w_pos_step[:,1],\
			...
			fig_test_name = f'./meshgraphnets/figs/case_{i}_time step_{time_step}_graph_translation_z.jpg'
			plt.savefig(fig_test_name)
...
# ################################ Visualizing the cross section ################################
			...
			plt.savefig(f'./meshgraphnets/figs/case_{i}_time step_{time_step}_data_interpolation_GS_{GS_sigma}')
			plt.close()
			# raise
...
#######################################################################################################3
		...
		# 	# print('world pos: ', w[time_step, :,0][:100])
			...
			fig_test_name = f'./meshgraphnets/figs/case_{i}_time step_{time_step}.jpg'
			plt.savefig(fig_test_name)
			# raise
			# raise
			# cbaxes = fig.add_axes([0.22, 0.55, 0.02, 0.15])
...
#######################################################################################################3
		...
		# np.save('./meshgraphnets/data/deforming_plate/mlsolver_x',\
		#  interpolated_grid_moving_delta_step_x_v)
		# np.save('./meshgraphnets/data/deforming_plate/mlsolver_y',\
		#  interpolated_grid_moving_delta_step_y_v)
		# np.save('./meshgraphnets/data/deforming_plate/mlsolver_z',\
		#  interpolated_grid_moving_delta_step_z_v)
