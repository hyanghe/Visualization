...
video_name = 'Tile_heating.avi'
zero_pad_width = 10
# video = cv2.VideoWriter(video_name, 0, 1, (height+zero_pad_width*2, (width+zero_pad_width*2)*2))
Time = np.random.uniform(0, 1, 201)
Temperature_org = np.load('./data_decay/Final_temp_decay.npy')

# Make empty black image
image_temp = np.zeros((num_pixel, num_pixel, 3), np.uint8)

with imageio.get_writer('./movie_decay.gif', mode='I') as writer:
    x_ranges = []
    y_ranges = []
    for i in range(1, num_step+1):
        globals()[f'new_img_{i}'] = np.zeros((height, width, 3), np.uint8)
        globals()[f'new_img_{i}'][:, :, 2] = 255
        step_size =  (1 - i / (num_step)) * 255
        # tile_x_coord = int(0.45*num_pixel):int(0.55*num_pixel) + i / (num_step) * (num_pixel//2)
        # tile_y_coord = int(0.45*num_pixel):int(0.55*num_pixel)
        tile_x_coord_s = int(0.45 * num_pixel)
        tile_x_coord_e = int(0.55 * num_pixel)
        tile_y_coord_s = int(0.45*num_pixel) + int(i / (num_step) * (num_pixel//2))
        tile_y_coord_e = int(0.55*num_pixel) + int(i / (num_step) * (num_pixel//2))

        globals()[f'new_img_{i}'][tile_x_coord_s:tile_x_coord_e, tile_y_coord_s:tile_y_coord_e, 0] = step_size
        globals()[f'new_img_{i}'][tile_x_coord_s:tile_x_coord_e, tile_y_coord_s:tile_y_coord_e, 1] = 0
        globals()[f'new_img_{i}'][tile_x_coord_s:tile_x_coord_e, tile_y_coord_s:tile_y_coord_e, 2] = 0
        # globals()[f'new_img_{i}'] = img
        globals()[f'new_img_{i}'] = cv2.putText(globals()[f'new_img_{i}'], f'Distance from center: {2000/num_step*i:.3f}', org_time, font, fontScale, color, thickness, cv2.LINE_AA)

        img_title = cv2.putText(globals()[f'new_img_{i}'], 'Near field to far field', org_tile, font, fontScale, color, thickness, cv2.LINE_AA)
        ...
        # Make one pixel red
        x = 500 // num_step * i
        Temperature = Temperature_org - Temperature_org.min()
        ...
        globals()[f'new_image_temp_{i}'] = cv2.putText(globals()[f'new_image_temp_{i}'], f'Temperature: {Temperature_org[i]:.3f}', org_temp, font, fontScale, color, thickness, cv2.LINE_AA)
